...
abstract class RopeSimulationCore {
    ...
    static void beginParallelPhase() { PARALLEL_PHASE.set(true); }
    static void endParallelPhase()   { PARALLEL_PHASE.set(false); }
    protected static boolean parallelPhase()  { return PARALLEL_PHASE.get(); }
    ...
    // in stacked ropes. Filled by preparePhysicsParallel; read by rope-rope constraints.
    ...
    public boolean matchesLength(Vec3 a, Vec3 b) {
        ...
        // does not trigger a full sim re-creation every tick, which would otherwise wipe
