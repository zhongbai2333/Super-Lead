...
/**
 * Per-step cache of world-space block collision boxes keyed by packed BlockPos. Cleared each
 * outer step before the constraint loop so all inner iterations share box lookups for free.
 */
final class BlockCollisionCache {
    ...
    /** Pre-fill the cache for every BlockPos in the inclusive bbox by querying {@code level}.
     *  Must be called on the main thread; after this returns, worker threads can safely call
