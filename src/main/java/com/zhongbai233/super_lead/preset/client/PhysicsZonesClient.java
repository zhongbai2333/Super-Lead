...
public final class PhysicsZonesClient {
    ...
    public static SyncPhysicsZones.Entry findAt(double x, double y, double z) {
        for (SyncPhysicsZones.Entry zone : ZONES) {
            if (contains(zone, x, y, z)) return zone;
        }
        return null;
    }

    /** Resolve a rope to the first zone that contains its midpoint, or otherwise intersects
     *  the endpoint segment. This keeps the fast, stable midpoint rule for overlaps while still
     *  matching ropes that are visibly inside a zone but whose midpoint lies on a boundary. */
    public static SyncPhysicsZones.Entry findForRope(Vec3 a, Vec3 b) {
        ...
        for (SyncPhysicsZones.Entry zone : ZONES) {
            if (segmentIntersects(zone, a, b)) return zone;
        }
        return null;
    }
    ...
    private static boolean segmentIntersects(SyncPhysicsZones.Entry zone, Vec3 a, Vec3 b) {
        if (contains(zone, a.x, a.y, a.z) || contains(zone, b.x, b.y, b.z)) return true;
        double t0 = 0.0D;
        double t1 = 1.0D;
        double dx = b.x - a.x;
        double dy = b.y - a.y;
