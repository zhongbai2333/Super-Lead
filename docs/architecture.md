...
- SavedData 维护按 `LeadKind` 隔离的运行时 revision；红石信号和 ITEM/FLUID/PRESSURIZED 路由索引只受自身类型更新影响，避免 ENERGY 发光、其他类型附件或视觉变化造成跨类型缓存重建。THERMAL 拓扑同样按自身 generation 缓存；热量按真实路径的逐绳剩余容量调度，串联取瓶颈，且端点交换遵循 Mekanism 的热容、逆导热与事务守恒规则。
